...
import { Bird, Truck, ShieldCheck } from 'lucide-react';
import { FlockTable } from '@/components/flocks/flock-table';
import { toFlockViewModel } from '@/lib/flocks';
...
export default async function AdminFlocksPage() {
  ...
  const [flocks, totalFlocks, deliveryEnabled, countrywide] = await Promise.all([
    prisma.livestockFlock.findMany({
      include: {
        vaccinationsGiven: true,
        medicationsGiven: true,
    ...
    prisma.livestockFlock.count({ where: { deliveryAvailable: true } }),
    prisma.livestockFlock.count({ where: { deliveryScope: 'COUNTRYWIDE' } }),
  ]);
  ...
  return (
    <DashboardLayout user={user}>
      <DashboardContent>
        <DashboardSection>
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">Global flock registry</h1>
            <p className="text-sm sm:text-base text-muted-foreground">
              Monitor all internal livestock records across seller accounts.
            </p>
          </div>
        </DashboardSection>
        ...
        <DashboardGrid cols={{ sm: 1, md: 3 }} gap="md">
          ...
          <DashboardCard>
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-muted-foreground">Delivery enabled</p>
                <p className="text-2xl font-bold">{deliveryEnabled}</p>
              </div>
          ...
          <DashboardCard>
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-muted-foreground">Countrywide</p>
                <p className="text-2xl font-bold">{countrywide}</p>
              </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>All seller flocks</CardTitle>
          </CardHeader>
          <CardContent>
            <FlockTable flocks={flockViewModels} basePath="/admin/flocks" description="Admin view across the full livestock registry." />
          </CardContent>
        </Card>
      </DashboardContent>
    </DashboardLayout>
