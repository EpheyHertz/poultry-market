...
export default async function CompanyDashboard() {
  ...
  const companyProductFilter = { items: { some: { product: { sellerId: user.id } } } };
  ...
  const [
    ...
    prisma.order.count({ where: companyProductFilter }),
    prisma.order.count({
      where: { ...companyProductFilter, status: { in: [...OPEN_ORDER_STATUSES] } },
    }),
    prisma.order.aggregate({
      where: { ...companyProductFilter, status: 'DELIVERED' },
      _sum: { total: true },
    }),
    prisma.order.findMany({
      where: companyProductFilter,
      include: {
        customer: {
          select: {
            name: true,
    ...
    prisma.sponsorship.count({ where: { companyId: user.id } }),
  ]);
  ...
  const stats = [
    ...
    {
      title: 'Sponsorships',
      value: sponsorships.toLocaleString(),
      icon: HandHeart,
      description: 'Sponsorships created',
    },
  ];

  const quickActions = [
    { href: '/company/products/new', label: 'Add product', icon: Plus },
    { href: '/company/products', label: 'Manage products', icon: Package },
    { href: '/company/orders', label: 'View orders', icon: ShoppingCart },
    { href: '/company/sponsorships', label: 'Sponsorships', icon: HandHeart },
    { href: '/company/analytics', label: 'Analytics', icon: TrendingUp },
  ...
  return (
    <DashboardLayout user={user}>
      <div className="space-y-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Manage your company products and track your business performance.
            </p>
          </div>
        ...
        <ApiKeyManager
          title="API Keys"
          description="Generate and manage keys for corporate integrations, ERPs, or reporting tools."
        />
      </div>
    </DashboardLayout>
  );
